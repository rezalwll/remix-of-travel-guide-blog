...
export default function SupportCenter() {
  ...
  return (
    <main id="main-content" className="min-h-[70vh] bg-muted/35">
      <TravelHero asset={serviceAsset("support", "تصویر همراهی و پشتیبانی سفر")} size="compact" eyebrow="مرکز راهنمای کیاشی" title="برای ادامهٔ سفر تنها نیستی" description="پاسخ سریع دربارهٔ خرید، پرداخت، پیگیری و استرداد. برای اطلاعات سفارش، شناسهٔ پیگیری را آماده داشته باشید." />

      <section className="media-section"><div className="container-page"><SectionHeader eyebrow="مسیر سریع" title="برای چه چیزی کمک می‌خواهی؟" /><div className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{categories.map(({ href, icon: Icon, title, text }) => <Link key={href} href={href} className="group rounded-2xl border bg-card p-5 transition-colors hover:border-secondary/40 hover:shadow-[var(--shadow-card)]"><span className="grid size-11 place-items-center rounded-xl bg-secondary/10 text-secondary"><Icon className="size-5" /></span><h2 className="mt-4 font-black group-hover:text-primary">{title}</h2><p className="mt-2 text-xs leading-6 text-muted-foreground">{text}</p></Link>)}</div></div></section>
