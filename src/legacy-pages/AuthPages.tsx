...
const AuthCard = ({ children, title, subtitle }: { children: React.ReactNode; title: string; subtitle: string }) => <Layout><main className="flex min-h-[78vh] items-center justify-center bg-muted/40 px-4 py-10"><section className="grid w-full max-w-5xl overflow-hidden rounded-[1.5rem] border border-border bg-card shadow-[var(--shadow-card)] lg:grid-cols-[1fr_1.05fr]"><div className="p-6 sm:p-9 lg:p-12"><div className="mb-7"><img src="/kiashi-logo.png" alt="کیاشی" className="h-12 w-auto max-w-[132px] object-contain object-right" /><h1 className="mt-5 text-2xl font-extrabold sm:text-3xl">{title}</h1><p className="mt-2 text-sm leading-7 text-muted-foreground">{subtitle}</p></div>{children}</div><div className="relative hidden min-h-[38rem] lg:block"><MediaFrame asset={photoLibrary.bloggers} ratio="4/5" sizes="50vw" overlay="strong" decorative className="absolute inset-0 size-full"><div className="absolute inset-x-0 bottom-0 z-10 p-10 text-white"><p className="text-xs font-bold text-accent">حساب سفر کیاشی</p><h2 className="mt-3 text-3xl font-black leading-tight">برنامه، سفارش و همراهان سفر در یک جای امن</h2><p className="mt-3 text-sm leading-7 text-white/80">ورود با کد یک‌بارمصرف انجام می‌شود؛ اطلاعات حساس بانکی در حساب ذخیره نمی‌شود.</p></div></MediaFrame></div></section></main></Layout>;
