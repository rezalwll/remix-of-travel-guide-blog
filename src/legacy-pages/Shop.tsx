    ...
    <Layout>
      <main className="pb-20">
        ...
        <section className="container-page pt-16 sm:pt-20"><div className="mb-8 max-w-2xl"><span className="section-eyebrow">انتخاب راهنما</span><h2 className="page-heading">ابزارهای جمع‌وجور برای سفرهای واقعی</h2><p className="mt-3 text-sm leading-7 text-muted-foreground">محتوای این فروشگاه نمایشی است و برای تکمیل تجربهٔ محصول ارائه می‌شود.</p></div>
          {visible.length ? <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{visible.map((product) => <article key={product.id} className="image-card group flex flex-col"><Link to={`/shop/${product.id}`} className="relative aspect-[4/3] overflow-hidden"><img src={product.image} alt={product.name} className="size-full object-cover" loading="lazy" /><span className="absolute end-3 top-3 rounded-full bg-white/90 px-2.5 py-1 text-[11px] font-extrabold shadow">{product.format}</span></Link><div className="flex flex-1 flex-col p-5"><div className="flex items-center gap-1 text-xs text-muted-foreground"><Star className="size-4 fill-accent text-accent" /><span className="font-extrabold text-foreground">{product.rating.toLocaleString("fa-IR")}</span><span>· {product.pages.toLocaleString("fa-IR")} صفحه</span></div><h3 className="mt-3 text-lg font-black leading-7"><Link to={`/shop/${product.id}`}>{product.name}</Link></h3><p className="mt-2 line-clamp-3 text-sm leading-7 text-muted-foreground">{product.description}</p><div className="mt-auto flex items-end justify-between gap-3 border-t border-border pt-4"><div><p className="text-[11px] text-muted-foreground">قیمت نسخه دیجیتال</p><p className="font-black text-primary">{formatPrice(product.price)}</p></div><Link to={`/shop/${product.id}`} aria-label={`مشاهده ${product.name}`} className="grid size-10 place-items-center rounded-xl bg-primary text-white"><ArrowLeft className="size-4" /></Link></div></div></article>)}</div> : <div className="surface-card py-16 text-center"><Search className="mx-auto size-8 text-muted-foreground" /><h2 className="mt-4 font-extrabold">راهنمایی با این عبارت پیدا نشد</h2><p className="mt-2 text-sm text-muted-foreground">عبارت کوتاه‌تری جست‌وجو کن.</p></div>}
        </section>
      </main>
    </Layout>
  );
