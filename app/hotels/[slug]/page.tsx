...
export default async function HotelLandingPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main id="main-content" className="min-h-[70vh] bg-muted/35">
      <TravelHero asset={destinationMedia} eyebrow="راهنمای اقامت · موجودی و نرخ فقط در جست‌وجوی واقعی" title={item.title} description={item.summary} badges={["انتخاب محله", "شرایط لغو", "هماهنگی با پرواز"]} overlapBottom />
      ...
      <section className="media-section pt-5"><div className="container-page"><SectionHeader eyebrow="حال‌وهوای اقامت" title={`پیش از رزرو در ${item.city}`} description="نوع اقامت را با برنامهٔ روزانه، مسیر رفت‌وآمد و ساعت ورود هماهنگ کنید." /><div className="mt-7"><ImageMosaic label={`گالری الهام‌بخش اقامت در ${item.city}`} assets={[destinationMedia, stayMedia.city, stayMedia.boutique]} /></div></div></section>
      ...
      <section className="media-section"><div className="container-page"><SectionHeader eyebrow="سبک اقامت" title="هر سفر، یک انتخاب متفاوت" /><div className="media-grid sm:grid-cols-2 lg:grid-cols-3"><ImageCard href={`/hotels/search?destination=${item.slug}`} asset={stayMedia.resort} title="اقامت تفریحی" description="برای سفرهای آرام و زمان بیشتر در محل اقامت." chips={["تصویر نمایشی", "بررسی امکانات"]} cta="جست‌وجو" /><ImageCard href={`/hotels/search?destination=${item.slug}`} asset={stayMedia.city} title="هتل شهری" description="برای دسترسی ساده‌تر به حمل‌ونقل و برنامه‌های روزانه." chips={["تصویر نمایشی", "بررسی موقعیت"]} cta="جست‌وجو" /><ImageCard href={`/hotels/search?destination=${item.slug}`} asset={stayMedia.traditional} title="اقامت متفاوت" description="برای تجربهٔ معماری و فضای محلی؛ موجودی در جست‌وجو مشخص می‌شود." chips={["تصویر نمایشی", "بدون ادعای موجودی"]} cta="جست‌وجو" /></div></div></section>

      <section className="media-section-tinted"><div className="container-page grid gap-5 lg:grid-cols-[.8fr_1.2fr]"><aside className="rounded-2xl border bg-card p-6"><ShieldCheck className="size-6 text-secondary" /><h2 className="mt-3 text-xl font-black">این صفحه چه چیزی را ادعا نمی‌کند؟</h2><p className="mt-3 text-sm leading-7 text-muted-foreground">قیمت، ظرفیت، امتیاز و موجودی زنده در دادهٔ ساختاریافتهٔ این صفحه منتشر نمی‌شود. اطلاعات تجاری فقط پس از پاسخ معتبر تأمین‌کننده قابل اتکاست.</p><Link href={item.destinationPath} className="secondary-cta mt-5">راهنمای سفر به {item.city}</Link></aside><PromoBanner href={item.destinationPath} asset={destinationMedia} eyebrow="شناخت مقصد" title={`${item.city} فقط محل اقامت نیست`} description="راهنمای زمان سفر، رفت‌وآمد و دیدنی‌ها را پیش از انتخاب محله ببین." cta="راهنمای مقصد" /></div></section>

      <section className="pb-14 sm:pb-20"><div className="container-page"><PromoBanner href="/support" asset={serviceAsset("support", "تصویر پشتیبانی رزرو اقامت")} eyebrow="راهنمای خرید" title="دربارهٔ شرایط اقامت سؤال داری؟" description="شرایط لغو و اطلاعات مهمان را پیش از پرداخت بررسی کن." cta="مرکز راهنما" align="center" /></div></section>
    </main>
  );
}
