...
export function generateStaticParams() { return seoRoutes.filter((item) => isIndexableContent(item, 5)).map((item) => ({ slug: item.slug })); }

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params;
  const route = getRouteLanding(slug);
  return route ? createMetadata({ title: `${route.title} | مقایسه پروازها`, description: route.description, path: `/flights/${slug}`, index: isIndexableContent(route, 5) }) : {};
}
